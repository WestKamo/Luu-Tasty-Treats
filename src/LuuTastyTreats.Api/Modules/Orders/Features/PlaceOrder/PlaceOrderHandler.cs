...
public class PlaceOrderHandler : IRequestHandler<PlaceOrderCommand, PlaceOrderResult> {
    ...
    public async Task<PlaceOrderResult> Handle(PlaceOrderCommand command, CancellationToken ct) {
        ...
        // 3. Process and Validate Order Items (Cakes)
        ...
        foreach (var (itemInput, index) in request.Items.Select((x, i) => (x, i))) {
            ...
            var allValidOptionIds = applicableGroups.SelectMany(cg => cg.Group.Options).Select(o => o.OptionId).ToList();
            ...
            // 4. Validate Custom Text Fields (e.g., messages written on the cake)
            ...
            var snapshotJson = JsonSerializer.Serialize(selections.Select(s => new { group = s.GroupNameSnapshot, option = s.OptionLabelSnapshot }));
